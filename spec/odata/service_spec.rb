...
describe OData::Service, vcr: {cassette_name: 'service_specs'} do
  ...
  it 'adds itself to OData::ServiceRegistry on creation' do
    ...
    expect(OData::ServiceRegistry[service_url]).to be_nil

    service = OData::Service.open(service_url, name: 'ODataDemo')

    expect(OData::ServiceRegistry['ODataDemo']).to eq(service)
    expect(OData::ServiceRegistry[service_url]).to eq(service)
  end
  ...
  describe '#navigation_properties' do
    ...
    it { expect(subject.navigation_properties['Product']['Categories']).to be_a(OData::NavigationProperty) }
  end
